...
var parseAdvertiserInfo = function(body) {
  ...
  advertiserInfo.url = $('#viewProfilePicture a').attr('href');
  ...
  advertiserInfo.mobileOptimized = $('.mobileIcon').text().clean().indexOf('Yes') !== -1 ? true : false;
          ...
          zanoxAuth.request(pageUrl, 'GET', function(err, res) {
            ...
            $('table tr').each(function() {
              ...
              $(this).find('th').each(function() {
                col.push($(this).text().clean());
              });

              $(this).find('td').each(function(i) {
                item[col[i]] = $(this).text().clean();
              });
